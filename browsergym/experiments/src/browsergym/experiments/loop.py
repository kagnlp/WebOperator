...
env = None  # global env variable to be used in other modules
...
class StepInfo:
    ...
    def from_step(self, env: gym.Env, action: str, obs_preprocessor: callable):
        ...
        if self.task_info.get("error") is not None:
            self.terminated = False

        self.obs["last_action_error"] = self.task_info.get("error", self.obs["last_action_error"])
        # self.obs["http_requests"] = copy.deepcopy(getattr(env.unwrapped.page, "http_requests", []))

        if obs_preprocessor:
            self.obs = obs_preprocessor(self.obs)
        # env.unwrapped.page.http_requests = []

    def from_action(self, agent: Agent, env: gym.Env):
        self.profiling.agent_start = time.time()
        self.action, self.agent_info = agent.get_action(self.obs.copy(), env)
    ...
    def save_step_info(self, exp_dir, save_json=True, save_screenshot=True, save_som=False):
        ...
        if save_json:
            ...
            with open(exp_dir / "steps_info.json", "w") as f:
                json.dump(data, f, indent=4, cls=DataclassJSONEncoder)
            
            try:
                with open(exp_dir / "task_info.json", "w", encoding="utf-8") as f:
                    json.dump(self.task_info, f, indent=4, ensure_ascii=False)
            except Exception as e:
                print(f"Error saving task_info.json: {e}")
                print(f"Task info: {self.task_info}")
...
class ExpArgs:
    ...
    logging_level_stdout: int = logging.ERROR
    exp_id: str = None
    ...
    def run(self, checkpoint):
        ...
        try:
            ...
            # set_env(env)
            # print("Environment URL: ", env.unwrapped.page.url)
            
            while not step_info.is_done:  # set a limit
                logger.debug(f"Starting step {step_info.step}.")
                action = step_info.from_action(agent, env.unwrapped)
                ...
                logger.debug(f"Agent state saved.")
    ...
    def _set_logger(self):
        ...
        # output logging traces to a log file (UTF-8 safe)
        file_handler = logging.FileHandler(
            self.exp_dir / "experiment.log",
            encoding="utf-8"   # 👈 ensures UTF-8
        )
        ...
        # remove previous stream handlers (avoid duplicate console logs)
        ...
        # save reference (useful for closing later)
