...
INCLUDE_BID_ROLES = ("link", "button", "searchbox", "textbox", "combobox", "checkbox", "radio", "select", "option", "LabelText", "menuitem", "strong")
...
def merge_static_texts(flattened_str: str) -> str:
    merged_lines = []
    prev_indent = None
    static_buffer = []

    for line in flattened_str.splitlines():
        match = re.match(r"^(\t*)StaticText\s+['\"](.*)['\"]$", line)
        if match:
            indent, text = match.groups()
            # Don't merge StaticText with indent == "\t"
            if indent == "\t":
                # Flush previous buffer
                if static_buffer:
                    merged_lines.append(
                        f"{prev_indent}StaticText {repr(' '.join(static_buffer))}"
                    )
                    static_buffer = []
                    prev_indent = None
            ...
            if prev_indent == indent or prev_indent is None:
                static_buffer.append(text)
                prev_indent = indent
            else:
                merged_lines.append(
                    f"{prev_indent}StaticText {repr(' '.join(static_buffer))}"
...
def flatten_axtree_to_str(
    ...
    def dfs(node_idx: int, depth: int, parent_node_filtered: bool, parent_node_name: str) -> str:
        ...
        #     return ""  # skip alert nodes, they are usually not useful
    ...
    return merge_static_texts(tree_str) if optimize else tree_str
...
def overlay_som(
    ...
    for bid, properties in extra_properties.items():
        if properties["set_of_marks"] and properties["bbox"]:
            ...
            linedashed(draw, x0, y0, x1, y0, fill=(
                0, 0, 0, 255), width=linewidth)
            linedashed(draw, x1, y0, x1, y1, fill=(
                0, 0, 0, 255), width=linewidth)
            linedashed(draw, x1, y1, x0, y1, fill=(
                0, 0, 0, 255), width=linewidth)
            linedashed(draw, x0, y1, x0, y0, fill=(
                0, 0, 0, 255), width=linewidth)
