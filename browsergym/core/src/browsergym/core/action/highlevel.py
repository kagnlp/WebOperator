...
ACTION_SUBSETS = {
    ...
    "miniwob_all": [
        ...
        # FOCUS_ELEMENT_AND_TYPE_TEXT (and substitute for FOCUS_ELEMENT_AND_TYPE_FIELD)
        fill,
    ],
    # adapted from MiniWoB repo
...
class HighLevelActionSet(AbstractActionSet):
    ...
    DemoMode = typing.Literal["off", "default",
                              "all_blue", "only_visible_elements"]
        ...
        if subsets:
            for subset in subsets:
                if subset in ACTION_SUBSETS:
                    ...
                    raise ValueError(
                        f"Unknown high-level action subspace: {subset}")
...
"""
            ...
            # parse docstring
            description, examples = action_docstring_parser.parse_string(
                func.__doc__)
...
One single action to be executed. You can only use one action at a time."""
            ...
            for action_name in ["fill", "click", "mouse_click", "keyboard_type"]:
                if action_name in self.action_set:
                    picked_examples.extend(
                        self.action_set[action_name].examples)
