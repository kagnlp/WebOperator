...
demo_mode: Literal["off", "default",
                   "all_blue", "only_visible_elements"] = None
...
def stop(text: str):
    """
    Stops the current interaction and sends a message to the user.

    Examples:
        stop("Based on the results of my search, the city was built in 1751.")
    """
...
def type(bid: str, value: str, press_enter_after: bool):
    """
    Type text into a form field. It focuses the element and triggers an input event with the entered text.
    It works for <input>, <textarea> and [contenteditable] elements. Optionally, it can press Enter after typing.

    Examples:
        type('237', 'example value', True)
        type('45', "multi-line\\nexample", False)
        type('a12', "example with \\"quotes\\"", True)
    """
    elem = get_elem_by_bid(page, bid, demo_mode != "off")
    add_demo_mode_effects(
        page, elem, bid, demo_mode=demo_mode, move_cursor=False)

    def do(force: bool):
        if demo_mode != "off":
            delay = max(2000 / len(value), 10)
            elem.clear(force=force, timeout=5000)
            elem.type(value, delay=delay, timeout=5000)  # no timeout
            if press_enter_after:
                elem.press("Enter", timeout=5000)
        else:
...
def note(text: str):
    """
    Adds a note to the agent memory.
...
def fill(bid: str, value: str, press_enter_after: bool = False):
    """
    Fill out a form field. It focuses the element and triggers an input event with the entered text.
    It works for <input>, <textarea> and [contenteditable] elements. Optionally, it can press Enter after typing.

    Examples:
        fill('237', 'example value', True)
        fill('45', "multi-line\\nexample", False)
        fill('a12', "example with \\"quotes\\"", True)
    """
    # elem = get_elem_by_bid(page, bid, demo_mode != "off")
    # add_demo_mode_effects(
    #     page, elem, bid, demo_mode=demo_mode, move_cursor=False)
    ...
    def do(force: bool):
        if demo_mode != "off":
            delay = max(2000 / len(value), 10)
            elem.clear(force=force, timeout=30000)
            elem.type(value, delay=delay, timeout=30000)  # no timeout
            if press_enter_after:
                elem.press("Enter", timeout=30000)
                time.sleep(10)
        else:
            elem.fill(value, force=force, timeout=30000)
            if press_enter_after:
                elem.press("Enter", timeout=30000)
                time.sleep(10)
...
def check(bid: str):
    ...
    elem = get_elem_by_bid(page, bid, demo_mode != "off")
    add_demo_mode_effects(
        page, elem, bid, demo_mode=demo_mode, move_cursor=True)
...
def select_option(bid: str, options: str | list[str]):
    ...
    def do(force: bool):
        elem.select_option(options, force=force, timeout=30000)
...
def click(
    ...
    modifiers: list[Literal["Alt", "Control",
    ...
    # print(f"CLICKING {bid}")
    elem = get_elem_by_bid(page, bid, demo_mode != "off")
    add_demo_mode_effects(
        page, elem, bid, demo_mode=demo_mode, move_cursor=True)

    def do(force: bool):
        elem.click(button=button, modifiers=modifiers, force=force, timeout=30000)
...
def drag_and_drop(from_bid: str, to_bid: str):
    ...
    add_demo_mode_effects(page, from_elem, from_bid,
                          demo_mode=demo_mode, move_cursor=True)
    from_elem.hover(timeout=500)
    page.mouse.down()

    to_elem = get_elem_by_bid(page, to_bid, demo_mode != "off")
    add_demo_mode_effects(page, to_elem, to_bid,
                          demo_mode=demo_mode, move_cursor=True)
...
def scroll_xy(delta_x: float, delta_y: float):
    """
...
def scroll(direction: str):
    """
    Scroll vertically given a direction. The direction can be "up" or "down".
...
def scroll_at(x: int, y: int, dx: int, dy: int):
    ...
    # map coordinates to page coordinates
    dx, dy = map_coordinates(page, dx, dy)
...
def mouse_drag_and_drop(from_x: float, from_y: float, to_x: float, to_y: float):
    ...
    """
    from_x, from_y = map_coordinates(
        page, from_x, from_y)  # map coordinates to page coordinates
    # map coordinates to page coordinates
    to_x, to_y = map_coordinates(page, to_x, to_y)
...
def goto(url: str):
    ...
    while True:
        try:
            ...
            print("TimeoutError on goto, retrying...")
            time.sleep(10)
        except playwright.sync_api.Error as e:
            print(f"Error on goto: {e}, retrying...")
            time.sleep(10)
    time.sleep(10)
