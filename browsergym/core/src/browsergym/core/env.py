            ...
            self.observation_space = gym.spaces.Dict(
                {
                    ...
                    "http_requests": gym.spaces.Sequence(gym.spaces.Dict({
                            "method": Unicode(),
                            "url": Unicode(),
                            "header": AnyDict(),
                            "payload": Unicode(),
...
"""
        ...
        obs = self._get_obs()
        logger.debug("Observation extracted during reset.")
    ...
    def _get_obs(self):
        if self.use_raw_page_output:
            obs = {
                ...
                "http_requests": tuple(copy.deepcopy(getattr(self.page, "http_requests", []))),
                "elapsed_time": np.asarray([time.time() - self.start_time]),
            }
            self.page.http_requests = []
            return obs
        ...
        for retries_left in reversed(range(EXTRACT_OBS_MAX_TRIES)):
            try:
                ...
                logger.debug("DOM pre-extraction done")
                dom = extract_dom_snapshot(self.page)
                logger.debug("DOM snapshot extracted")
                axtree = extract_merged_axtree(self.page)
                logger.debug("AXTree merged and extracted")
                focused_element_bid = extract_focused_element_bid(self.page)
                logger.debug(f"Focused element bid extracted: {focused_element_bid}")
                extra_properties = extract_dom_extra_properties(dom)
                logger.debug("Extra DOM element properties extracted")
                break
                ...
                if retries_left > 0 and (
                    ...
                    raise e
            if last_axtree is not None:
                if len(axtree["nodes"]) != len(last_axtree["nodes"]):
                    logger.warning(
                        f"AXTree size changed during extraction ({len(last_axtree['nodes'])} -> {len(axtree['nodes'])} nodes). Retrying ({retries_left}/{EXTRACT_OBS_MAX_TRIES} tries left)."
                    )
                    # post-extract cleanup (ARIA attributes)
                    _post_extract(self.page)
                    time.sleep(1.0)
                    ...
                    logger.info(f"AXTree size stable during extraction: {len(last_axtree['nodes'])}")
                    break
            else:
                _post_extract(self.page)
                time.sleep(1.0)
        ...
        logger.debug("Post-extraction started...")

        # post-extraction cleanup of temporary info in dom
        _post_extract(self.page)
        
        logger.debug("Post-extraction done.")
