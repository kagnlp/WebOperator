...
class ProgressMessage(BaseMessage):
    ...
    def _get_system_message(self):
        ...
        elif self.prompt_type == "web_operator":
            system_message = {"role": "system", "content": JUDGE_WO_PROMPT_TEMPLATE["system"]}
        else:
    ...
    def _setup_input_information(self):
        ...
        if self.use_checklist:
            if self.prompt_type == "web_shepherd":
                ...
                input_information = USER_INSTRUCTION + TRAJECTORY + observation + CHECKLIST + CHECKLIST_COMPLETION + AGENT_RESPONSE
        else:
    ...
    def _get_user_message(self):
        ...
        if self.prompt_type == "web_shepherd":
            ...
            user_prompt = JUDGE_WO_PROMPT_TEMPLATE["user"].format(
                input_information=input_information,
            )
        else:
...
class ChecklistMessage(BaseMessage):
    ...
    def _get_system_message(self):
        if self.prompt_type == "web_shepherd" or self.prompt_type == "web_operator":
...
#                 "trajectory": [],
#                 "text_observation": 'No page content available',
#                 "current_url": "https://en.wikipedia.org/wiki/Main_Page",
#                 "action": "Click the link with text 'Paris' to navigate to the Paris information page.",
#                 "thought": "The action seems reasonable because it aligns with the user's goal.",
